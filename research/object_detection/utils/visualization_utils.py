...
def visualize_boxes_and_labels_on_image_array(
  ...
  return image
def visualize_boxes_and_labels_on_image_array_MOD(
    image,
    boxes,
    classes,
    scores,
      ...
      if scores is None:
        ...
        if not skip_labels:
          if not agnostic_mode:
            ...
            xmin = int(xmin*width)
            xmax = int(xmax*width)
            objects_detected.append([class_name,xmin, ymin, xmax, ymax ])
        ...
        if agnostic_mode:
          ...
          #prime_multipler = _get_multiplier_for_color_randomness()
          box_to_color_map[box] = track_ids[i]
        else:
          box_to_color_map[box] = STANDARD_COLORS[98]
    objcount +=1
